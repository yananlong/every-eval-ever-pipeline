...
class DuckDBBackend:
    ...
    Thread safety: all public methods are guarded by an instance-level ``threading.Lock``,
    making this class safe for concurrent use across multiple threads within a **single process**.
    Multi-process deployments must use separate ``DuckDBBackend`` instances (one per process).
    """
            ...
            matched_instances = int(
                self.conn.execute(
                    ...
                    WHERE EXISTS (
                        ...
                        WHERE m.evaluation_id = i.evaluation_id
                          AND m.result_join_id = i.result_join_id
                    )
                    """
                ).fetchone()[0]
            )
